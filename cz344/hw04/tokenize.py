...
for (num,(label,fn)) in enumerate(ind):
		...
		#cur = open(path+'/'+fn).read().replace('\n',' ').translate(replace_punctuation).split()
		cur = open(path+'/'+fn).read().replace('\n',' ').replace(',',' ').replace('?',' ').split()
		#cur = open(path+'/'+fn).read().replace('\n',' ').split()
		cur = [x.lower() for x in cur]
		#cur_new = [x for x in cur if x not in wordlist]
		# the next command loads in the data, replaces returns with blanks, splits it into words and hashes the words into integers
		email=map(lambda e: abs(e.__hash__()) % HASHBUCKETS,cur);
...
#			temp[x] = temp[x]+1;
		...
		# the next command loads in the data, replaces returns with blanks, splits it into words and hashes the words into integers
		#email=map(lambda e: abs(e.__hash__()) % HASHBUCKETS,cur_new);
		# write out "emailnumber,hashedword" for all words
		#sys.stdout.write('\n'.join(map(lambda a: '%i,%i' % (num+1,a+1),email))+'\n');
